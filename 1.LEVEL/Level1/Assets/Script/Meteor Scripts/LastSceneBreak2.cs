...
public class LastSceneBreak2 : MonoBehaviour
{
    //Dialogues
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F) && RockInDialogueSize == true && LastSceneDialogue.firstF==true)
        {
            RockDialogueVirtualCamera.GetComponent<CinemachineVirtualCamera>().LookAt = NPC.transform;
            RockDialogueVirtualCamera.GetComponent<CinemachineVirtualCamera>().Priority = 255;
            Fireworks.SetActive(true);
            Smoke.transform.DOScale(new Vector3(0, 0, 0), 4);
            Smoke.transform.DOMoveY(999, 8);
            StartCoroutine(destroySmoke());
            _lookAt.transform.DOMoveY(150, 25);
